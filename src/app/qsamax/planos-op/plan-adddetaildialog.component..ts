...
@Component({
  selector: 'app-plan-adddetaildialog',
   standalone: true,
  imports: [
    CommonModule,
    MatSelectModule,
  ...
  templateUrl: './plan-adddetaildialog.component.html',
    styleUrls: ['./plan-adddetaildialog.component.scss']
})
export class planAddDetailDialogComponent {
  form: FormGroup;
 ...
 ops$: Observable<WorkOrder[]> = of([]);
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<planAddDetailDialogComponent>,
  ...
  submit() {
    ...
    formData.append('plan_type', this.form.value.plan_type);
    formData.append('plan_review', this.form.value.plan_review);
    formData.append('uploaded_by', this.currentUserEmail || 'anon');

    const url = `${environment.apiUrlqsa}/subir-detalle/${this.data.planId}`;
    this.http.post(url, formData).subscribe({
      next: (res) => {
        this.dialogRef.close(true);
      },
      error: (err) => {
        console.error('Error al subir detalle:', err);
        alert('Error al subir el detalle del plano.');
      }
    });
  }
